...
export const WORKFLOW_MARKDOWN = [
  ...
  "6. When asked to sync project state, run oflow sync --summary --json for the token-light overview. Use oflow sync --json only when full bounded planning collections or pagination are needed. Use oflow sync --cached --summary --json for repeated context when the last snapshot is acceptable, and --refresh when current GitLab state is required. Add --epics only when group-level epic context is needed; it costs one extra bounded GraphQL read. When assessing one story, run oflow assess --story <iid> --json and use oflow capabilities --json to discover supported operations.",
