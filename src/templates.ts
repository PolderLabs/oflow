...
export const WORKFLOW_MARKDOWN = [
  ...
  "6. When asked to sync project state, run oflow sync --json. Use oflow sync --cached --json for repeated context when the last snapshot is acceptable, and --refresh when current GitLab state is required. Add --epics only when group-level epic context is needed; it costs one extra bounded GraphQL read. When assessing one story, run oflow assess --story <iid> --json and use oflow capabilities --json to discover supported operations.",
...
export const OFLOW_README_MARKDOWN = [
  ...
  "- Live sync writes a credential-free ignored cache; --cached never contacts GitLab and reports the snapshot age. Refresh before relying on current state or preparing a remote write.",
...
export function agentInstructionBlock(agent: AgentName): string {
  ...
  return [
    ...
    "use oflow sync --json for project state (use --cached for an acceptable local snapshot, --refresh when current GitLab state is required; add --epics for group epics), oflow iteration --state current --json for a focused sprint view, oflow assess --story <iid> --json",
